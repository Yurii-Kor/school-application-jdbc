...
public class StudentsCoursesDao {
	private static final String INSERT_STUDENTS_COURSE = String.format("INSERT INTO %s (%s, %s) VALUES (?, ?)",
			DBSchemaConstants.STUDENTS_COURSES_TABLE.getValue(),
			DBSchemaConstants.STUDENTS_COURSES_STUDENT_ID.getValue(),
			DBSchemaConstants.STUDENTS_COURSES_COURSE_ID.getValue());
		...
		try (Connection connection = ConnectionPool.getConnection();
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				while (resultSet.next()) {
					courseIds.add(resultSet.getLong(DBSchemaConstants.STUDENTS_COURSES_COURSE_ID.getValue()));
				}
			}
	...
	public List<Long> findStudentIdsByCourseId(Long courseId) throws DAOException {
		...
		try (Connection connection = ConnectionPool.getConnection();
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				while (resultSet.next()) {
					studentIds.add(resultSet.getLong(DBSchemaConstants.STUDENTS_COURSES_STUDENT_ID.getValue()));
				}
			}
